...
class NimbleScraper:
    ...
    def fetch_page(self, url, render=True, wait_time=5000):
        ...
        # Add render_flow to wait for page to fully load
        if render:
            payload["render_flow"] = [
                {
                    "wait": {
    ...
    def fetch_page_with_spa_pagination(self, url, page_number=1, wait_time=5000):
        """
        Fetch a specific page from Zillow's React SPA by manipulating the searchQueryState
        
        Args:
            url (str): Base URL (page 1)
            page_number (int): Which page to fetch (1-indexed)
            wait_time (int): Time to wait after navigation (milliseconds)
        ...
        # Build render_flow for SPA pagination
        render_flow = [
        ...
        # If requesting page > 1, we need to trigger pagination via JavaScript
        if page_number > 1:
            # Step 2: Execute JavaScript to update searchQueryState and trigger re-render
            js_code = f"""
            (function() {{
                // Find the searchQueryState in the page
                var scriptTag = document.querySelector('script#__NEXT_DATA__');
                if (scriptTag) {{
                    var nextData = JSON.parse(scriptTag.textContent);
                    var searchState = nextData.props?.pageProps?.searchPageState?.queryState;
                    ...
                    if (searchState) {{
                        ...
                        searchState.pagination.currentPage = {page_number};
                        
                        // Trigger Zillow's internal navigation
                        // This updates the URL and triggers data fetch
                        var newUrl = window.location.pathname + '?searchQueryState=' + encodeURIComponent(JSON.stringify(searchState));
                        window.history.pushState(null, '', newUrl);
                        
                        // Trigger React router/state update
                        window.dispatchEvent(new PopStateEvent('popstate'));
                        
                        return 'Pagination updated to page {page_number}';
                    }}
                }}
                return 'Could not find searchQueryState';
            }})();
            """
            ...
            # Step 3: Wait for React to re-render with new data
            render_flow.append({
                "wait": {
                    "delay": wait_time
                }
        ...
        payload["render_flow"] = render_flow
        
        try:
            print(f"Fetching: {url[:100]}... (Page {page_number})")
            response = requests.post(
                self.api_url,
                headers=self.headers,
                json=payload,
